...
class MyBroker {
    ...
    async submit(price, size, side, waitTime, cancel) {
        ...
        let params = side == 'buy' ? { "reduceOnly": true } : null;
...
; (async () => {
    ...
    while (true) {
        ...
        let trading = false;
        if (positionAmt < 0) {
            let price = status == 'down' ? bidPrice : (ask1 - priceGrain);
            let order = await broker.buy(price, -positionAmt, 200, false);
            if (order.status == "open") {
                orderId = order.orderId;
                closePrice = price;
            }
            else {
                price < costPrice*0.992 ? trader.win() : trader.lose();
                costPrice = 0;
                closePrice = 0;
            }
            trading = true;
