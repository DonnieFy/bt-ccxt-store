...
class MyData:
    ...
    def calc_changes(self, pos, start_time):
        changes = 0
        count = len(self._data)
        for i in range(count):
            ohlcv = self._data[count - i - 1]
            if ohlcv[0] < start_time:
                break
            change = ohlcv[4] - ohlcv[1]
            if pos > 0 > change:
                changes += change
            elif pos < 0 < change:
                changes += change
...
class MyBroker:
    ...
    def get_stop_loss(self, data):
        ...
        start_time = int((self.position_dates[name] - datetime(1970, 1, 1)).total_seconds() * 1000)
        changes = data.calc_changes(pos=pos, start_time=start_time)
        return self.position_stop_loss[name] - changes
    ...
    def open_position(self, d, date, side, pnt, interval):
        ...
        self.logger.log('value：%s, cash: %s' % (total_value, cash))
        self.logger.log('symbol：%s, side: %s, pnt: %s, interval: %s' % (symbol, side, pnt, interval))

        ratio = (total_value // 15)
        ...
        self._submit(symbol=symbol, exectype='Market', side=side, amount=size, price=d.price(), stop_loss=self.position_stop_loss[symbol])
    ...
    def _submit(self, symbol, exectype, side, amount, price, stop_loss=None):
        self.logger.log('symbol: {}, side: {}, price: {}, size: {}'.format(symbol, side, price, amount))
        params = {
            'stopLossPrice': stop_loss
        }
        ret_ord = self.exchange.create_order(symbol=symbol, type=exectype, side=side, amount=amount, price=price, params=params)
...
def main(debug=False, test=False):
    ...
    while True:
        try:
            ...
            long_600 = btc.price() > btc.calc_sma(600, 0)
            ...
            for market in datas:
                ...
                if long_450 and long_450_1 and long_450_2:
                    ...
                    if 0.12 >= pnt >= 0.06 and data.calc_avg(15, 0) >= 10 and data.calc_agree(15, 0) >= 0.8:
                    ...
                    elif 0.12 >= pnt >= 0.01 and data.calc_pnt(15, 1) >= 0.01 and data.calc_pnt(15, 2) >= 0.01 and \
                    ...
                    pnt = data.calc_pnt(45, 0)
                    if 0.15 >pnt > 0.04 and data.calc_pnt(15, 0) > data.calc_pnt(15, 15) > data.calc_pnt(15, 30)\
                            and data.price(0) > data.calc_sma(300, 0) > data.calc_sma(1200, 0):
                        broker.open_position(d=data, date=current_date, side='buy', pnt=pnt, interval=45)
                        continue
            ...
            for d in set(broker.holds()):
                ...
                if size > 0:
                    if interval > broker.get_stop_interval(name) or d.price() < broker.get_stop_loss(d):
