...
class MyTrader {
    ...
    complete(openPrice, closePrice, status) {
        if (status == "up") {
...
; (async () => {
    ...
    while (true) {
        ...
        if (orderId) {
            ...
            if (!canceled) {
                // 已经执行了，开始下一个买卖
                trader.complete(costPrice, closePrice, status);
                continue;
            }
            ...
            if (positionAmt < 0) {
                ...
                if (order.status == "open") {
                    ...
                    trader.complete(costPrice, price, status);
                    costPrice = 0;
                    closePrice = 0;
                }
            }
            ...
            if (canceleds[0] && canceleds[1]) {
                logger.log("status: %s, bidPrice: %d, askPrice: %d", "cancel", bidPrice, askPrice);
            }
            else if (!canceleds[0] && !canceleds[1]) {
                logger.log("status: %s, bidPrice: %d, askPrice: %d", "close", bidPrice, askPrice);
                trader.win();
            }
            else if (canceleds[0] && !canceleds[1]) {
                status = "down";
                logger.log("status: %s, bidPrice: %d, askPrice: %d", "down", bidPrice, askPrice);
                costPrice = askPrice;
                closePrice = askPrice * 0.9992;
                let order = await exchange.createLimitOrder(symbol, 'buy', size, closePrice);
                orderId = order.id;
            }
            else {
                status = "up";
                logger.log("status: %s, bidPrice: %d, askPrice: %d", "up", bidPrice, askPrice);
                costPrice = bidPrice;
                closePrice = bidPrice * 1.0008;
                let order = await exchange.createLimitOrder(symbol, 'sell', size, closePrice);
                orderId = order.id;
            }
            trading = true;
        }
