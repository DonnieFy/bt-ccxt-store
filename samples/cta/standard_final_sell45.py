    ...
    def next(self):
        ...
        for d in self.datas:
            if d in self.position_datas:
                continue
            if short:
                pnt = self.inds[d]['pnt_45'][-30]
                up = d.close[-30] - d.close[-75]
                down = self.inds[d]['high_30'][0] - d.close[0]
                if pnt > 0.04 and 0.3 <= down / up and self.inds[d]['pnt_15'][0] < self.inds[d]['pnt_15'][-15] < 0\
                        and self.inds[d]['sma_300'][0] < self.inds[d]['sma_1200'][0]:
                    self.open_position(d=d, date=current_date, side='buy', pnt=pnt, interval=15, position_type='sell45')
