...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.last_1200_dates = dict()
        self.last_sma_cross_dates = dict()
        ...
        for d in self.datas:
            ...
            # self.inds[d]['sma_450'] = bt.indicators.SMA(d, period=450)
            # self.inds[d]["low"] = bt.indicators.Lowest(d.low, period=120)
            # self.inds[d]["high"] = bt.indicators.Highest(d.high, period=120)
    ...
    def next(self):
        ...
        long_450 = self.datas[0].close[0] > self.sma_450[0]
        long_450_1 = self.datas[0].close[-1] > self.sma_450[-1]
        long_450_2 = self.datas[0].close[-2] > self.sma_450[-2]
        long_888 = self.datas[0].close[0] > self.sma[0]
        long_888_1 = self.datas[0].close[-1] > self.sma[-1]
        long_888_2 = self.datas[0].close[-2] > self.sma[-2]
        for d in self.datas:
            if d in self.position_datas:
                continue
            name = d._name
            if self.inds[d]['sma_300'][0] < self.inds[d]['sma_1200'][0] < self.inds[d]['sma_300'][-1]:
                self.last_sma_cross_dates[name] = current_date

            if d.close[0] < self.inds[d]['sma_1200'][0] < d.close[-1]:
                self.last_1200_dates[name] = current_date

            if name in self.last_1200_dates and name in self.last_sma_cross_dates \
                    and self.inds[d]['sma_300'][0] < self.inds[d]['sma_1200'][0] \
                    and d.close[0] < self.inds[d]['sma_1200'][0]:
                interval = (current_date - self.last_1200_dates[name]).seconds / 60
                interval2 = (current_date - self.last_sma_cross_dates[name]).seconds / 60
                pnt = self.inds[d]['pnt_15'][0]
                if interval2 >= 1200 and interval < 60 and pnt <= -0.02:
                    self.open_position(d=d, date=current_date, side='sell', pnt=pnt, interval=60, position_type='sell60')
        ...
        for d in set(self.position_datas):
            ...
            if size < 0:
                if interval >= self.position_stop_interval[name] \
                        or d.close[0] > self.position_stop_loss[name]:
    ...
    def open_position(self, d, date, side, pnt, interval, position_type):
        ...
        if total_value > cash or len(self.position_datas) >= 3:
            return
        pnt = abs(pnt)
    ...
    def close_position(self, data, size, side):
        ...
        pnl = (data.close[0] - self.position_prices[name]) * size
        if name not in self.position_type_win_time:
            self.position_type_win_time[name] = 0
        if name not in self.position_type_lose_time:
            self.position_type_lose_time[name] = 0
        if name not in self.position_type_win_money:
            self.position_type_win_money[name] = 0
        if name not in self.position_type_lose_money:
            self.position_type_lose_money[name] = 0

        if pnl > 0:
            self.position_type_win_time[name] += 1
            self.position_type_win_money[name] += pnl
        else:
            self.position_type_lose_time[name] += 1
            self.position_type_lose_money[name] += pnl
...
# symbols = ['HNT/USDT'] # , 'SFP/USDT', 'XTZ/USDT', 'LINK/USDT', 'KLAY/USDT']
