...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        for d in self.datas:
            ...
            # self.inds[d]['sma'] = bt.indicators.SMA(d, period=888)
    ...
    def need_buy(self, d):
        ...
        return self.inds[d][const_pnt][0] > const_factor * sum(pnts) / len(pnts) and self.calc_ratio(d, 0) >= 0.8
    ...
    def next(self):
        ...
        for d in set(self.position_datas):
            ...
            if size > 0:
                if (interval > 5 and self.calc_ratio(d, 0) < 0.75) or d.close[0] <= self.position_stop_loss[name]:
        ...
        for d in act_buys:
            ...
            self.ana(d)
            ss = total_value * (0.01/abs(pnt)) / d.close[0]
    ...
    def open_buy(self, data, size, exectype, date):
        ...
        self.position_stop_profit[name] = data.close[0] * (1 - pnt)
