...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.sma = bt.indicators.SMA(self.data, period=888)
        self.sma_450 = bt.indicators.SMA(self.data, period=450)
        for d in self.datas:
            self.inds[d] = dict()
    ...
    def next(self):
        ...
        for d in self.datas:
            ...
            if long_450 and long_450_1 and long_450_2:
                ...
                high_name = 'high_15'
                low_name = 'low_15'
                if pnt >= 0.01 and self.calc_avg(d, 0, pnt_name, 15) >= 10 and self.calc_agree(d, 0, pnt_name, high_name, low_name) >= 0.8:
                    # btc>450sma，15分钟>=0.06，15分钟>10*avg6，agree>0.8，1次确认做多，持仓60分钟
                    self.open_position(d=d, date=current_date, side='buy', pnt=pnt, interval=60)
                if pnt >= 0.005 and self.inds[d]['pnt_15'][-1] >= 0.01 and self.inds[d]['pnt_15'][-2] >= 0.01 and \
                    ...
                    self.open_position(d=d, date=current_date, side='buy', pnt=pnt, interval=15)
                ...
                if -0.05 <= pnt_15 <= -0.04 and -0.05 <= self.inds[d]['pnt_15'][-1] <= -0.04 and \
                    ...
                    self.open_position(d=d, date=current_date, side='sell', pnt=pnt_15, interval=15)
            ...
            if not long_888 and not long_888_1:
                pnt_45 = self.inds[d]['pnt_45'][0]
                if pnt_45 >= 0.06 and self.calc_avg(d, 0, 'pnt_45', 45) >= 10 and self.calc_agree(d, 0, 'pnt_45', 'high_45', 'low_45') >= 0.8:
                    # btc<888sma，45分钟>=0.06，45分钟>=10*avg6，agree>=0.8，1次确认做空，持仓60分钟
                    self.open_position(d=d, date=current_date, side='sell', pnt=pnt_45, interval=60)
            # elif long_888 and long_888_1 and long_888_2:
            #     pnt_30 = self.inds[d]['pnt_30'][0]
            #     if pnt_30 >= 0.03 and self.inds[d]['pnt_30'][-1] >= 0.03 and self.inds[d]['pnt_30'][-2] >= 0.03 and \
            ...
            #         self.open_position(d=d, date=current_date, side='buy', pnt=pnt_30, interval=45)
    ...
    def open_position(self, d, date, side, pnt, interval):
        ...
        total_value = (30000 * ratio if ratio > 0 else 30000)
        if total_value > cash:
            return
...
symbol = 'BTC/USDT'
data = store.getdata(dataname=symbol, name=symbol, market='future',
