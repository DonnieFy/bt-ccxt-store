...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.position_candidate = dict()
        self.position_last_date = dict()
        self.logger = logging.getLogger()
        self.logger.setLevel(logging.INFO)
        self.pre_sells = []
        self.pre_sells2 = []
        handler = logging.FileHandler('logfile_final_1.log', encoding='UTF-8')
        ...
        for d in self.datas:
            ...
            # self.inds[d]['sma_80'] = bt.indicators.SMA(d, period=210)
    ...
    def next(self):
        ...
        btc = self.datas[0]
        sma_80 = self.calc_sma(btc, 15, 80)
        short = btc.close[0] < sma_80 and self.calc_sma(btc, 15, 20) < sma_80
        for d in self.datas:
            ...
            if d in self.position_datas:
                continue
            name = d._name
            if name in self.position_last_date and (current_date - self.position_last_date[name]).seconds / 60 < 75:
                continue
            pnt = self.inds[d]['pnt_15'][0]
            if short and current_date.minute in [14, 29, 44, 59]:
                if pnt <= -0.02:
                    sma_1200 = self.calc_sma(d, 15, 80)
                    if d.close[0] < sma_1200 and self.calc_sma(d, 15, 20) < sma_1200\
                            and -5 <= self.calc_avg(d, 0, 'pnt_15', 15) <= -2:
                        self.open_position(d=d, date=current_date, side='sell', pnt=pnt, interval=15, position_type='sell15')
    ...
    def open_position(self, d, date, side, pnt, interval, position_type):
        ...
        self.position_last_date[name] = date
