...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.pre_buys = []
        self.pre_buys2 = []
        handler = logging.FileHandler('logfile_break.log', encoding='UTF-8')
        handler.setLevel(logging.INFO)
        self.logger.addHandler(handler)
        self.sma = bt.indicators.SMA(self.datas[0], period=888)
        for d in self.datas:
            self.inds[d] = dict()
            self.inds[d]['high_15'] = bt.indicators.Highest(d.close, period=888)
            self.inds[d]['low_15'] = bt.indicators.Lowest(d.close, period=888)
    ...
    def next(self):
        ...
        for d in self.datas:
            if d in self.position_datas:
                ...
                continue
            if long and d.close[0] >= self.inds[d]['high_15'][0] >= self.inds[d]['low_15'][0]*1.03:
                buys.append(d)
        ...
        for d in set(self.position_datas):
            ...
            if size > 0:
                if interval > 240 or d.close[0] <= self.position_stop_loss[name] or d.close[0] >= self.position_stop_profit[name]:
    ...
    def update_stop(self, data):
        ss = ''
        # name = data._name
        # if data.close[0] > self.position_high[name] > 0:
        #     self.position_stop_loss[name] += data.close[0] - self.position_high[name]
        #     self.position_high[name] = data.close[0]
    ...
    def open_buy(self, data, size, exectype, date):
        ...
        # self.position_stop[name] = data.close[0] * (1 - 0.05)
        # self.position_high[name] = 0
        low = self.inds[data]['low_15'][0]
        self.position_stop_loss[name] = low
        self.position_stop_profit[name] = data.close[0] + 3 * (data.close[0] - low)
