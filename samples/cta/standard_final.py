...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.position_current_date = set()
        ...
        for d in self.datas:
            ...
            self.inds[d]['sma_210'] = bt.indicators.SMA(d, period=210)
    ...
    def next(self):
        ...
        self.position_current_date.clear()
        ...
        btc = self.datas[0]
        # long = self.sma[0] < self.sma_300[0] < btc.close[0] and self.sma[-1] < self.sma_300[-1] < btc.close[-1] and self.sma[-2] < self.sma_300[-2] < btc.close[-2]
        # long = btc.close[0] > self.sma_300[0] and btc.close[-1] > self.sma_300[-1] and btc.close[-2] > self.sma_300[-2]
        long = self.sma_300[0] > self.sma[0] and self.sma_300[-1] > self.sma[-1] and self.sma_300[-2] > self.sma[-2]
        short = btc.close[0] < self.sma_300[0] < self.sma[0] and self.sma_300[-1] < self.sma[-1] and self.sma_300[-2] < self.sma[-2]
        long = False
        for d in self.datas:
            ...
            if long:
                ...
                if pnt > 0.04 and self.inds[d]['pnt_15'][0] > self.inds[d]['pnt_15'][-15] > self.inds[d]['pnt_15'][-30]\
                        and d.close[0] > self.inds[d]['sma_300'][0] > self.inds[d]['sma_1200'][0]:
                    self.open_position(d=d, date=current_date, side='buy', pnt=pnt, interval=45, position_type='buy45')
                    continue

                pnt = self.inds[d]['pnt_15'][0]
                if pnt >= 0.01 and self.inds[d]['pnt_15'][-1] >= 0.01 and self.inds[d]['pnt_15'][-2] >= 0.01 and \
                        self.calc_avg(d, 0, pnt_name, 15) >= 10 \
                        and self.calc_avg(d, -1, pnt_name,15) >= 10 \
                        and self.calc_avg(d, -2,pnt_name, 15) >= 10 \
            ...
            if short:
                ...
                if pnt < -0.02 and -5 <= self.calc_avg(d, 0, 'pnt_15', 15) <= -2 \
                        ...
                        and d.close[0] < self.inds[d]['sma_1200'][0] < d.close[-15]:
                    self.open_position(d=d, date=current_date, side='sell', pnt=pnt, interval=45,
                                       position_type='sell45')
                    continue
                ...
                if -0.05 <= pnt_60 <= -0.04 and \
                        ...
                        self.inds[d]['sma_300'][0] < self.inds[d]['sma_1200'][0] and \
                    ...
                    self.open_position(d=d, date=current_date, side='sell', pnt=pnt_60, interval=30, position_type='sell30')
                    continue
        ...
        for d in set(self.position_datas):
            ...
            if size < 0:
                pnt = d.close[0] / self.position_prices[name] - 1
                if d not in self.postion_data_add and 20 >= interval >= 15 and pnt <= -0.02:
                    self.postion_data_add.add(d)
                    o_interval = self.position_stop_interval[name]
                    self.open_position(d=d, date=current_date, side='sell', pnt=pnt, interval=o_interval, position_type='addsell' + str(o_interval))
                elif interval >= self.position_stop_interval[name] or d.close[0] > self.position_stop_loss[name]:
                    self.close_position(data=d, size=size, side='buy')
                    if d in self.postion_data_add:
                        self.postion_data_add.remove(d)
            elif size > 0:
                pnt = d.close[0] / self.position_prices[name] - 1
                # if d not in self.postion_data_add and 20 >= interval >= 15 and pnt >= 0.02:
                #     self.postion_data_add.add(d)
                #     o_interval = self.position_stop_interval[name]
                #     self.open_position(d=d, date=current_date, side='buy', pnt=pnt, interval=o_interval, position_type='addbuy' + str(o_interval))
    ...
    def open_position(self, d, date, side, pnt, interval, position_type):
        ...
        if total_value > cash or total_value > cash - total_value * len(self.position_current_date):
            return
        ...
        self.position_current_date.add(d)
