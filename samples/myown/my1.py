...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.global_ind = Global(self.datas)
...
# 设置交易手续费为 0.1%
cerebro.broker.setcommission(commission=0.008, mult=1)
...
hist_start_date = datetime.utcnow() - timedelta(minutes=15 * 4 * 24 * 1)
data0 = None
for v in symbols:
    data = store.getdata(dataname=v, name=v, market='future',
                     timeframe=bt.TimeFrame.Minutes, fromdate=hist_start_date,
                     compression=15, ohlcv_limit=None, drop_newest=True, historical=True)
    # Add the feed
