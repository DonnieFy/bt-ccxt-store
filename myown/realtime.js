...
class MyBroker {
    ...
    openPosition(trade) {
        ...
        if (side == 'buy' && pos_size < 0 || side == 'sell' && pos_size > 0) {
            ...
            Array.from(this._symbol_trades[symbol]).forEach(trade => {
                LOGGER.log('SIDE CHANGED, info: %s', JSON.stringify({ symbol, side, size, posType }));
            ...
            promise = Promise.all([exchange.cancelAllOrders(symbol), this.getSize(symbol)]).then(res => {
                pos_size = res[1];  // 实时查仓位，有可能刚好前一秒止损了导致仓位不见了
    ...
    logProfitLoss(symbol, orderId, positionType) {
        let since = exchange.milliseconds() - PERIOD_15M * 10;
        return exchange.fetchMyTrades(symbol, since).then(trades => {
    ...
    while (true) {
        try {
            ...
            if (quarter == preQuarter) {
                let newMinute = preMinutes != minutes;
                let newHours = preHours != hours;
                preMinutes = minutes;
                preHours = hours;
                ...
                if (newHours && (hours % 4 == 0)) {
                    broker.print();
                }
                if (newMinute && minutes % 7 == 0) {
                    await Promise.all(datas.map(d => d.fetchMaxAmount()));
                }
                ...
                for (let i = currentPositionTrades.length - 1; i >= 0; i--) {
                    ...
                    if (exchange.milliseconds() > trade['stopDate'] || (openSide == 'buy' && d.getClose(0) < stopLoss) || (openSide == 'sell' && d.getClose(0) > stopLoss)) {
                        ...
                        // 止损单没发出去
                        let stopLossSide = openSide == 'buy' ? 'sell' : 'buy';
                        closePromises.push(exchange.createMarketOrder(symbol, stopLossSide, size, 0, { 'stopPrice': stopLoss }).then(stopLossOrder => {
                            trade['stopLossOrderId'] = stopLossOrder.id;
                        }))
                    }
                    else if (newMinute) {
                        // 每分钟检查下止损是否触发，避免方向变换的时候还不知道仓位已经没了
                        closePromises.push(exchange.fetchOrder(stopLossOrderId, symbol).then(order => {
                            if (order.status == "closed") {
                                broker.stopLoss(symbol, positionType);
                                broker.removeTrade(trade);
